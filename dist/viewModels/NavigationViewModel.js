...
var NavigationViewModel = function (options) {
  ...
  this.enableZoomControls = defined(options.enableZoomControls)
    ...
    : true;
  this.enableReset = defined(options.enableReset) ? options.enableReset : true;
...
NavigationViewModel.prototype.show = function (container) {
  ...
  const enableCompassOuterRing =
    this.terria.options.enableCompassOuterRing !== undefined
      ? this.terria.options.enableCompassOuterRing
      : true;

  const compassPre = '<div class="compass"';
  const compassPreAfter = enableCompassOuterRing
  ...
  const compassOuterRingBackground =
    '<div class="compass-outer-ring-background"></div>';

  const compassRotationMarkerPre =
  ...
  const compassRotationMarkerDefaultSvg =
    compassRotationMarkerPre +
    ', cesiumSvgPath: { path: svgCompassRotationMarker, width: 145, height: 145 }"' +
    divCloseStr;
  const compassRotationMarkerSelf =
    compassRotationMarkerPre +
    '"' +
    divCloseStr +
    this.terria.options.compassRotationMarkerSvg; //self define svg
  const compassRotationMarker =
    (this.terria.options.compassRotationMarkerSvg
      ? compassRotationMarkerSelf
      : compassRotationMarkerDefaultSvg) + closeStr;

  const compassOuterRingPre =
    " <div class=\"compass-outer-ring\" title=\"\" data-bind=\"style: { transform: 'rotate(-' + heading + 'rad)', '-webkit-transform': 'rotate(-' + heading + 'rad)' }";
  const compassOuterRingDefaultSvg =
    compassOuterRingPre +
    ', cesiumSvgPath: { path: svgCompassOuterRing, width: 145, height: 145 }"' +
    divCloseStr;
  const compassOuterRingSelf =
    compassOuterRingPre +
    '"' +
    divCloseStr +
    this.terria.options.compassOuterRingSvg; //self define svg
  const compassOuterRing =
    (this.terria.options.compassOuterRingSvg
      ? compassOuterRingSelf
      : compassOuterRingDefaultSvg) + closeStr;

  const compassGyroBackground = ' <div class="compass-gyro-background"></div>';
  const compassGyroPre =
    ' <div class="compass-gyro" data-bind="css: { \'compass-gyro-active\': isOrbiting }';
  const compassGyroDefaultSvg =
    compassGyroPre +
    ',cesiumSvgPath: { path: svgCompassGyro, width: 145, height: 145 } "' +
    divCloseStr;
  const compassGyroSelf =
    compassGyroPre + '"' + divCloseStr + this.terria.options.compassGyroSvg;
  const compassGyro =
    (this.terria.options.compassGyroSvg
      ? compassGyroSelf
      : compassGyroDefaultSvg) + closeStr;

  const compassDivPublicStr =
    compassPreAfter +
  ...
  const navigationControlsDivPublicStr =
    ...
    '  <div class="navigation-control-icon-svg" zoomin>' +
    zoomInSvg +
    "</div>" +
    "  <!-- /ko -->" +
    "  <!-- ko ifnot: $data.zoomInSvg -->" +
    "  <!-- ko if: $data.zoomOutSvg -->" +
    '  <div class="navigation-control-icon-svg zoomout">' +
    zoomOutSvg +
    "</div>" +
    "  <!-- /ko -->" +
    "  <!-- /ko -->" +
  ...
  const navigationControlsDivStr =
    navigationControlsPre + navigationControlsDivPublicStr;
  const navigationControlsDivHiddenStr =
    navigationControlsPre +
    hiddenStr +
    divCloseStr +
    navigationControlsDivPublicStr;

  const testing =
    (this.enableCompass ? compassDivStr : compassDivHiddenStr) +
    (this.enableZoomControls
      ? navigationControlsDivStr
      : navigationControlsDivHiddenStr);
